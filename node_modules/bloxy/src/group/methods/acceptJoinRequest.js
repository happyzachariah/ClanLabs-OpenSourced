...
exports.run = async function (requestIdentifier) {
	...
	if (response.statusCode !== 200) throw new Error(`Failed to accept join request. ${response.status}`);
	if (response.body.success != true) throw new Error(`Failed to accept join request. ${response.status}`);
